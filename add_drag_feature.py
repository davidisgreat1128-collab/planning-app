...
# 1. 在四象限任务项添加长按和拖拽事件
# 找到四个象限的任务项模板,添加 @longpress 和其他拖拽事件

# Q3 - 紧急不重要
content = re.sub(
    r'(<view\s+v-for="task in filteredUrgentNotImportant"[^>]*class="nb-task-item"[^>]*@tap="openTaskDetail\(task\)">)',
...
# Q1 - 重要且紧急
content = re.sub(
    r'(<view\s+v-for="task in filteredUrgentImportant"[^>]*class="nb-task-item"[^>]*@tap="openTaskDetail\(task\)">)',
...
# Q4 - 不重要不紧急
content = re.sub(
    r'(<view\s+v-for="task in filteredNotUrgentNotImportant"[^>]*class="nb-task-item"[^>]*@tap="openTaskDetail\(task\)">)',
...
# Q2 - 重要不紧急
content = re.sub(
    r'(<view\s+v-for="task in filteredNotUrgentImportant"[^>]*class="nb-task-item"[^>]*@tap="openTaskDetail\(task\)">)',
    r'\1\n                @longpress="onTaskLongPress($event, task, \'q2\')"\n                @touchmove="onTaskTouchMove"\n                @touchend="onTaskTouchEnd"',
    content,
    count=1
)

# 2. 在四象限视图区域后添加拖拽蒙层和删除区域
...
# 替换 </view><!-- end quadrant-view --> 并添加删除区域
content = re.sub(
    r'(</view><!-- end quadrant-view -->)',
    delete_zone_template,
    content,
    count=1
)

# 3. 在CategoryDrawer组件前添加两个对话框

# 更改象限确认对话框
change_quadrant_dialog = '''
    <!-- 更改象限确认对话框 -->
...
content = re.sub(
    r'(\s*<!-- 规划和分类抽屉 -->)',
    change_quadrant_dialog + r'\1',
    content,
    count=1
)

# 4. 在script中添加状态变量 (在userCategories后)
drag_state_vars = '''
...
content = re.sub(
    r"(const isFirstShow = ref\(true\);[^\n]*\n)",
    drag_state_vars,
    content,
    count=1
)

# 5. 在toggleTaskDone函数后添加拖拽相关函数
drag_functions = '''
...
# 在toggleTaskDone函数后添加拖拽函数
# 先找到toggleTaskDone函数
toggle_task_done_pattern = r'(async function toggleTaskDone\(task\) \{[^}]*\})'
match = re.search(toggle_task_done_pattern, content, re.DOTALL)
if match:
    insert_pos = match.end()
    content = content[:insert_pos] + drag_functions + content[insert_pos:]
else:
    print("Warning: toggleTaskDone函数未找到,将在文件末尾添加拖拽函数")

# 6. 添加CSS样式 (在文件末尾的</style>前)
drag_styles = '''
/* ============================================================
   拖拽相关样式
   ============================================================ */
...
print("Added:")
print("  - Longpress and drag events for quadrant tasks")
print("  - Drag overlay and delete zone UI")
print("  - Change quadrant confirmation dialog")
print("  - Delete task confirmation dialog")
print("  - Drag state management and functions")
print("  - CSS styles")
