...
1. [治理目标与原则](#1-治理目标与原则)
2. [单一职责原则（SRP）核心要点](#2-单一职责原则srp核心要点) ⭐⭐⭐ 最高优先级
3. [四层架构职责边界](#3-四层架构职责边界)
4. [AI编码前的6个必查项](#4-ai编码前的6个必查项)
5. [AI编码的7步标准流程](#5-ai编码的7步标准流程)
6. [代码复用与复杂度控制](#6-代码复用与复杂度控制)
7. [常见违规场景与纠正](#7-常见违规场景与纠正)
8. [架构演进与维护](#8-架构演进与维护)
...
## 2. 单一职责原则（SRP）核心要点 ⭐⭐⭐ 最高优先级

> **完整版参见**: [企业级单一职责原则(SRP)指南](./企业级单一职责原则(SRP)指南.md)
> **本章定位**: AI快速参考手册（简化版）

### 8.1 SRP三句话速记

1. **定义**: 一个组件只应该有一个改变的理由
2. **判断**: 用"责任清单法"列出职责，职责>3个即违规
3. **成本**: 违反SRP导致修改成本×3、返工成本×2、学习成本×5

---

### 8.2 AI必须执行的SRP检查（修改代码前）
...
| 症状 | 阈值 | 解决方案 |
|------|------|---------|
| 文件过大 | >800行（Component） | 使用"责任清单法"拆分 |
| 函数过长 | >50行 | 提取子函数 |
| 依赖过多 | import >20个 | 拆分组件 |
| 命名含"and/Manager" | 如`TaskAndCategoryManager` | 拆分为独立组件 |
| 需要注释分割模块 | `// ===== 任务部分 =====` | 每个注释块应是独立组件 |
| 修改一处影响多处 | 改表单验证破坏提交逻辑 | 按职责拆分 |
| 测试难以编写 | 需Mock 3+个依赖 | 职责过重，必须拆分 |
...
```
步骤1：列出组件的所有职责（3-5分钟）
  示例：template/detail.vue 有11个职责
  - 显示模板头部、显示用户信息、显示里程碑、管理弹窗...
...
步骤2：决策
  → 违反SRP且预计修改>100行 → 先拆分再修改
  → 违反SRP且预计修改<100行 → 先修改，完成后立即拆分
  → 不违反SRP → 正常修改
...
**📚 详细说明**: [企业级单一职责原则(SRP)指南.md](./企业级单一职责原则(SRP)指南.md)（500行完整版）
...
# 步骤2：使用工具检查
# 运行ESLint或手动检查文件中是否有格式错误：
grep -n "^\s*\*[^/]" <文件路径>  # 查找可能缺少 /** 的行
```
...
### 7.2 违规场景2：Composable 直接调用 Repository
