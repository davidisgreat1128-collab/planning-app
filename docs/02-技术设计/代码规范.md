...
## 8. 文件大小约束（2026-03-03新增）

### 硬性阈值
| 文件类型 | 触发提醒阈值 | 说明 |
|---------|------------|------|
| Vue页面组件 | 800行 | 超过后必须登记到追踪清单 |
| Vue通用组件 | 800行 | 同上 |
| Service层 | 800行 | 同上 |
| Controller层 | 800行 | 同上 |
| Store模块 | 800行 | 同上 |
...
### 管理原则
1. **所有拆分必须经用户批准**：Claude 不得自作主张拆分文件
2. **追踪机制**：所有超标文件必须登记到 `docs/02-技术设计/超标文件追踪清单.md`
3. **不打断工作流**：发现超标时，先完成当前功能，再提醒用户
4. **Bug修复优先**：修复紧急Bug时可以先修复，修复后再标记文件
...
| 场景 | 推荐方案 |
|------|---------|
| Vue组件多视图/多Tab | 拆成多个子组件，用 `<component :is>` 动态渲染 |
| 复杂表单 | 拆成独立 Form 组件 |
| 弹窗/抽屉过多 | 每个弹窗独立成组件 |
| 业务逻辑复杂 | 抽取为 Composable（`use*.js`） |
| Service 方法过多 | 按领域拆分为多个 Service |
| Store 状态过多 | 拆分为多个 Store 模块 |

### Claude 工作流程
详见 `.claude/CLAUDE.md` 第 7.8 节"文件大小管理规范"
