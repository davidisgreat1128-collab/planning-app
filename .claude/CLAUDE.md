...
### 7.10 备份文件管理规范 ⭐ (新增，2026-03-10)

#### 核心原则
- **命名规范**: `<原文件名>.backup-<日期>-<目的>-<状态>.<扩展名>`
- **存放位置**: 统一存放在 `backups/` 目录，**禁止与源文件同目录**
- **强制登记**: 所有备份文件必须在 `backups/README.md` 中登记

#### 强制规范

**禁止行为 ❌**
- ❌ 读取 backups/ 目录中的文件作为当前代码
- ❌ 备份文件与源文件放在同一目录
- ❌ 备份文件命名不清晰（缺少日期、目的、状态）
- ❌ 创建备份后不在 backups/README.md 中登记

**必须执行 ✅**
- ✅ 复杂重构前必须创建备份（文件>1000行 或 预计改动>500行）
- ✅ 备份文件必须按规范命名（日期+目的+状态）
- ✅ 备份文件必须存放在 backups/ 目录
- ✅ 创建备份后立即在 backups/README.md 中登记
- ✅ 读取代码前检查是否是备份文件
...
# 2. 复制文件到backups目录并重命名
cp pages/calendar/task-edit.vue backups/pages/calendar/task-edit.vue.backup-20260310-迁移save函数前-进行中.vue

# 3. 在 backups/README.md 中登记
```

**场景2：读取代码时检查**
```
在使用 Read 工具前检查：
  文件路径包含 "backups/" 或 ".backup" → 这是备份文件！
  → 必须向用户确认："检测到这是备份文件，是否需要查看历史版本？"
  → 用户未确认前，禁止读取
```

**场景3：重构完成后清理备份**
```
步骤1：更新 backups/README.md，将备份从"进行中"移到"已完成"
步骤2：询问用户是否删除备份
步骤3：用户确认后删除备份文件
```
...
- 📄 [备份文件管理规范.md](../docs/02-技术设计/备份文件管理规范.md) - 完整规范和使用指南
- 📄 [重构状态追踪清单.md](../docs/02-技术设计/重构状态追踪清单.md) - 重构进度追踪
- 📄 [backups/README.md](../backups/README.md) - 备份文件登记清单
...
> **适用范围**: 所有代码修改、新增、重构场景
>
> 💡 **完整规范**：详见 [AI工程治理规范.md](../docs/02-技术设计/AI工程治理规范.md)（~1500行完整版，含详细说明、代码示例、违规案例）

### 8.0 SRP（单一职责原则）核心原则 ⭐ 必读

> **完整指南**: [企业级单一职责原则(SRP)指南](../docs/02-技术设计/企业级单一职责原则(SRP)指南.md)
> **本节定位**: 5句话速记版

#### SRP五句话速记

1. **定义**: 一个组件只应该有一个改变的理由（一个职责）
2. **判断**: 列出职责清单，职责>3个 → 违反SRP
3. **成本**: 违反SRP导致修改成本×3、返工成本×2、学习成本×5
4. **检测**: 文件>800行、函数>50行、命名含"and/Manager" → 可能违反SRP
5. **拆分**: 使用"责任清单法"按"谁会要求修改"分组，每组独立成组件
...
|------|---------|---------|
| 这个组件有几个改变的理由？ | 1个 | 3+个 |
| 修改某功能时，影响几个组件？ | 只影响1个 | 需要改3+个 |
| 命名能否用一个名词描述？ | `TaskEditor` | `TaskAndCategoryManager` |

#### SRP快速自查（提交代码前）

- [ ] Component <800行且只负责1个业务实体
- [ ] Composable <600行且只负责1个业务流程
- [ ] Utils函数只做1件事且无副作用
- [ ] 文件命名不含"and/Manager"
- [ ] 修改某功能只影响1个文件
...
### 8.1 修改代码前的6个必查项 ⭐ 强制执行

**在开始编写任何代码之前，必须完成以下6项检查。违反任一项将导致架构退化。**
...
**检查问题**：我的JSDoc注释格式是否正确？是否会导致JS编译错误？
...
JSDoc注释格式错误会导致：
- ❌ Vite编译失败："Failed to parse source for import analysis because the content contains invalid JS syntax"
- ❌ 代码无法正常运行
- ❌ 浪费大量时间排查语法错误
...
```
□ 我已完成6个必查项（架构、文件大小、字段、三端兼容、文档、JSDoc）
...
| **修改代码前跳过第8节"6个必查项"** ⭐ | 导致架构退化、代码腐化、技术债务累积（第8节） |
...
| 运行ESLint | 提交代码前 |
| 使用JSDoc注释 | 编写每个函数时 |
| **执行第8节"6个必查项"** ⭐ | **修改任何代码之前**（架构、文件大小、字段、三端兼容、文档、JSDoc） |
