...
const TEMPLATE_DATABASE = {
  ...
  'tpl_3': {
    ...
    tasksByDay: {
      1: [
        { title: '6:30起床晨跑2公里', isRepeat: true, priority: 'high' },
        { title: '记录跑步数据', isRepeat: true, priority: 'medium' }
      ]
    }
  },
  'tpl_4': {
    ...
    tasksByDay: {
      1: [
        { title: '列出今日3件最重要的事', isRepeat: true, priority: 'high' },
        { title: '完成重要紧急任务', isRepeat: true, priority: 'high' },
        { title: '回顾今日完成情况', isRepeat: true, priority: 'medium' }
      ]
    }
  }
};
...
onMounted(() => {
  ...
  if (options.id) {
    ...
    // 从模板数据库加载模板数据
    const template = TEMPLATE_DATABASE[options.id];
    if (template) {
      templateData.value = JSON.parse(JSON.stringify(template)); // 深拷贝
      console.log('[TemplateDetail] 模板数据已加载:', templateData.value);
    } else {
      console.warn('[TemplateDetail] 未找到模板ID:', options.id);
      uni.showToast({
        title: '模板不存在',
        icon: 'none'
      });
