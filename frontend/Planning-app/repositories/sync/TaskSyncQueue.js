...
export class TaskSyncQueue {
  ...
  enqueue(operation) {
    ...
    console.log(`[TaskSyncQueue] 已添加操作到队列: ${queueItem.type} ${queueItem.entityId}`)
  }
  ...
  async sync(requestFn, onSuccess = null) {
    ...
    console.log(`[TaskSyncQueue] 开始同步 ${pending.length} 个操作...`)
    ...
    this._saveQueueToLocalStorage()
    console.log(`[TaskSyncQueue] 同步完成: 成功 ${successCount} 个，失败 ${failedCount} 个`)
