<template>
  <view v-if="visible" class="category-dialog-mask" @tap="onMaskTap">
    <view class="category-dialog" @tap.stop>
      <!-- 标题 -->
      <view class="dialog-title">分类名称</view>
      ...
      <view class="name-input-wrapper">
        <input
          ...
          :focus="inputFocus"
        />
        <text class="char-count">{{ categoryName.length }}/15</text>
      </view>
    ...
    <view v-if="showTutorial" class="tutorial-mask" @tap="closeTutorial">
      <view class="tutorial-dialog" @tap.stop>
        ...
        <view class="tutorial-content">
          <text class="tutorial-title">分类贴纸教程</text>
          <text class="tutorial-desc">将计划设置分类后，将会在四象限/单列表中显示该计划的分类贴纸</text>
        </view>
...
function openTutorial() {
  ...
  showIconPreview.value = false; // 关闭预览弹窗
}
...
function closeTutorial() {
  ...
  showIconPreview.value = true; // 返回预览弹窗
}
...
watch(() => props.visible, (val) => {
  if (val) {
    ...
    if (props.editMode && props.initialData) {
      // 编辑模式：加载现有数据
      categoryName.value = props.initialData.name || '';
      selectedIcon.value = props.initialData.icon || '';

      // 根据图标ID找到对应的分类标签
      const iconData = getIconById(props.initialData.icon);
      if (iconData) {
        // 解析图标ID的分类前缀 (如 "work_1" → "work")
        const tabKey = props.initialData.icon.split('_')[0];
